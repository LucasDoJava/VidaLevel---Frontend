...
export default function AddFriendModal({ isOpen, onClose }) {
  ...
  // 1) Carrega usuários quando o modal abre
  useEffect(() => {
    if (!isOpen) return;
    ...
    const fetchUsers = async () => {
      try {
        ...
        // Backend pode retornar ARRAY puro (seu caso atual) OU { users: [...] } (se você mudar)
  ...
  // 2) Filtra no front conforme digita
  ...
  async function handleAddFriend(receiverId) {
    try {
      ...
      // endpoint do backend que você já estava usando
